...
function scrollLoadingImg(scrTop, baseH, callback) {
    ...
    var ImgLoadedShow = function (img, imgSrc, dataSrc) {
        setTimeout(function () {
            ...
            //$$("body").innerHTML = $$("body").innerHTML + "<div>" + (img[i].querySelector(".DSbg").style.cssText).toString() + "</div>";
            //console.log("img", img, img.parentNode);
            if (img.parentNode.nodeName.toLowerCase() == "li") {
                if (DS.parentsUntil(img, "li").querySelector(".detailbox")) {
                    DS.parentsUntil(img, "li").querySelector(".detailbox").style.opacity = 1;
                }
                if (DS.parentsUntil(img, "li").querySelector(".distance")) {
                    DS.parentsUntil(img, "li").querySelector(".distance").style.opacity = 1;
                }
            } else if (DS.parentsUntil(img, ".designerbg")) {
                if (DS.parentsUntil(img, ".designerbg").querySelector(".designerbox")) {
                    DS.parentsUntil(img, ".designerbg").querySelector(".designerbox").style.opacity = 1;
                }
            }
        }, 500)
    ...
    var imgCanUse = function () {
        ...
        console.log("当前图片加载开始项imgScrollIndex:", imgScrollIndex);
        //只有向下滚动时或滑动时才会触发滚屏加载。
        for (var i = imgScrollIndex; i < imglen; i++) {
            ...
            if (img[i].getBoundingClientRect().top < baseH) {
                //console.log(i);
                ...
                if (dataSrc.getAttribute("loaded") != "true") {
                    ...
                    //dataSrc.setAttribute("src", imgSrc);
...
function controlLazyLaoding(dataAppendFunc, list, showcount, everycount) {
        ...
        if (DS.pagecomplete == undefined) {
            ...
            /*if (DS.pagecomplete == false) {
                ...
                var timer = setInterval(function () {
                    if(DS.pagecomplete == true || timercol > 600) {
                        clearInterval(timer);
                        //数据加载完毕取消loading动画
                        cssLoadingAnimate(false);
                    }
    ...
    if (DS.pagecomplete != undefined) {
        ...
        /*if (DS.pagecomplete == false) {
            var timercol = 0;
            var bodyHtimer = setInterval(function () {
                DS.bodyH = document.body.clientHeight;
                if(DS.pagecomplete == true || timercol > 600) {
                    clearInterval(bodyHtimer);
                }
                timercol++;
                console.log("nono")
            }, 50);
        ...
        //没有preloadbg图片 就不需要scrollLoadingImg这个方法
        if (!$$(".preloadbg")) {
            /*scrollLoadingImg = function () {
             eval(null);
             }*/
        }
    ...
    if (DS.pagecomplete == undefined) {
        ...
        /*var canScrollimg = setInterval(function () {
            if (DS.pagecomplete == true) {
                clearInterval(canScrollimg);
                scrollLoadingImg(0, baseH);
                console.log("a")
            }
        }, 50)*/
